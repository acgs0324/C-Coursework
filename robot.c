...
void displayRobot(robot robot, Cell grid[cell_num][cell_num]) {
    foreground();
    clear();
...
void forward(robot robot, Stack *step_record, Cell grid[cell_num][cell_num]) {
...
void left(robot* robot, Cell grid[cell_num][cell_num], Stack *step_record) {
    sleep(100);
...
void right(robot* robot, Cell grid[cell_num][cell_num], Stack *step_record) {
    sleep(100);
...
direction getInitDirection(char* direction) {
    ...
    switch (direction_c) {
        ...
        case 'w':
            return west;
        default:
            printf("Invalid starting direction\n");
            exit(1);
    }
}

void changeDirection(direction prev_robot_dir, direction robot_dir, robot* robot, Cell grid[cell_num][cell_num], Stack *step_record) {
...
int atHome(int robot_x_num, int robot_y_num, Cell grid[cell_num][cell_num]) {
    return grid[robot_x_num][robot_y_num].type == home;
}

int atMarker(robot robot, Cell grid[cell_num][cell_num]) {
    return grid[robot.x_num][robot.y_num].type == marker;
}

void pickUpMarker(robot* robot, Cell grid[cell_num][cell_num]) {
...
void dropMarker(robot* robot, Cell grid[cell_num][cell_num]) {
    sleep(200);
    robot->isCarryingAMarker = 0;
    displayRobot(*robot, grid);
}

void checkHomeAndResetStack(robot robot, Stack *step_record, Cell grid[cell_num][cell_num]) {
    for (int i = 0; i < 4; i++) {
...
void returnToHome(robot* robot, Stack *step_record, Cell grid[cell_num][cell_num]) {
...
int exploreBesides(robot* robot, Stack *step_record, Cell grid[cell_num][cell_num], type type) {
...
int depth_first_search(robot* robot, int prev_robot_x_num, int prev_robot_y_num, Stack *step_record, Cell grid[cell_num][cell_num]) {
